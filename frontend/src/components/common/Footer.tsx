...
const footerLinks = {
  ...
  forRestaurants: {
    ...
    titleAm: 'ለምግብ ቤቶች',
    links: [
      { label: 'Join as Partner', href: '/restaurant/register' },
      { label: 'Restaurant Dashboard', href: '/restaurant/dashboard' },
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto lg:min-w-96" aria-label="Newsletter">
      <input
        ...
        required
        className="flex-1 px-4 py-3 rounded-xl bg-white/10 border border-white/30 text-white placeholder:text-white/50 outline-none focus:border-white transition-all"
      />
      <Button
        type="submit"
        ...
        className="border border-white/30 text-white bg-white/10 hover:bg-white/20 shrink-0 px-6 py-3 rounded-xl font-semibold"
        animate={false}
      >
        Subscribe
      </Button>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-8">
            <div className="text-white text-center lg:text-left flex-1">
              ...
              <p className="text-white/70 text-sm mt-1">
                Subscribe to get exclusive offers, restaurant news, and food inspiration from across Ethiopia.
              </p>
            </div>
            <NewsletterForm />
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed">
              {APP_INFO.TAGLINE}. Connecting Ethiopian restaurants with hungry customers across 6 cities.
            </p>
              ...
              {socialLinks.map((social) => {
                ...
                return (
                  <motion.a
                    ...
                    whileTap={{ scale: 0.9 }}
                    className={cn('w-9 h-9 rounded-xl flex items-center justify-center text-muted-foreground bg-muted transition-all duration-200', social.color)}
          ...
          <div className="flex flex-wrap gap-2">
            {ETHIOPIAN_CITIES.map((city) => (
              <Link
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-muted text-sm text-muted-foreground hover:bg-primary/10 hover:text-primary border border-border hover:border-primary/30 transition-all duration-200"
              >
        ...
        <div className="mt-8 pt-8 border-t border-border">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex gap-3">
              <a href={APP_INFO.IOS_APP_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 rounded-xl bg-foreground text-background text-sm font-semibold hover:opacity-90 transition-opacity" aria-label="Download on App Store">
                🍎 App Store
              </a>
              <a href={APP_INFO.ANDROID_APP_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 rounded-xl bg-foreground text-background text-sm font-semibold hover:opacity-90 transition-opacity" aria-label="Get it on Google Play">
                🤖 Google Play
              </a>
            </div>
          </div>
